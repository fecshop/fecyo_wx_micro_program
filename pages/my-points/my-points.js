...
Page({
  ...
  getPointList: function () {
    ...
    if (requestStatus == 0) {
      ...
      wxRequestOrderStatus = 'offset'
    }
    
    //this.getOrderStatistics();
    wx.request({
      url: app.globalData.urls + '/customer/point/index',
      data: {
      ...
      success: (res) => {
        ...
        if (res.data.code == 200) {
          var pointList = res.data.data.point_historys;
          var point_amount = res.data.data.point_amount;
          var pointListThis = that.data.pointList;
          if (pointList) {
            for (var x in pointList) {
              pointListThis.push(pointList[x])
            }
          }
          that.setData({
            pointList: pointListThis,

...
Page({
  ...
  onLoad: function (e) {
    ...
    that.getWalletRechargeHistoryList();
  },
  onReady: function () {
    // 生命周期函数--监听页面初次渲染完成
  },
  getWalletRechargeHistoryList: function () {
    ...
    wx.request({
      url: app.globalData.urls + '/customer/wallet/rechargehistory',
      data: {
        p: that.data.page
      },
      ...
      success: (res) => {
        ...
        if (res.data.code == 200) {
          var rechargeOrders = res.data.data.recharge_orders;
          var rechargeOrdersThis = that.data.rechargeOrders;
          //var symbolCode = res.data.data.symbol_code;
          if (rechargeOrders) {
            for (var x in rechargeOrders) {
              rechargeOrdersThis.push(rechargeOrders[x])
            }
          }
          that.setData({
            rechargeOrders: rechargeOrdersThis,
